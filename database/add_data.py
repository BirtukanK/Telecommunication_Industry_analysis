...
def DBConnect(dbName=None):
    ...
    """
    conn = mysql.connect(host='localhost', user='root', password="w1x2y3z4@13",
...
def insert_to_info_table(dbName: str, df: pd.DataFrame, table_name: str) -> None:
    """
...
if __name__ == "__main__":
    createDB(dbName='telecom')
    emojiDB(dbName='telecom')
    createTables(dbName='telecom')

    df = pd.read_csv('../data/final_table.csv')

    insert_to_info_table(dbName='telecom', df=df, table_name='Final_information')